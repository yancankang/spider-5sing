...
def openUrl():
    ...
    print('%d.正在访问网页：%s.' % (globalValues.SUCCESS_COUNT, openingUrl))
    try:
        request = urllib.request.Request(openingUrl)
        response = urllib.request.urlopen(request, timeout=10)
        html = response.read().decode('utf-8', 'ignore')
        globalValues.SUCCESS_COUNT += 1
    except Exception as e:
        print('%s 抓取失败' % (openingUrl))
        print(e)
        globalValues.FAIL_COUNT+=1
        return
    finally:
        visited.add(openingUrl)
    for urlChild in linkRe.findall(html):
        urlChild not in visited and urlChild not in urlQueue and urlQueue.append(urlChild)
...
def writeSongMsg(html):
    ...
    if songName and msg:
        ...
        globalValues.SONG_PAGE_COUNT+=1
        if len(globalValues.SONG_MSG_BUFFER) > globalValues.BUFFER_SIZE:
            ...
            with open(filePath, encoding='utf-8', errors='ignore', mode='a') as result_file:
                print('写入歌曲信息, 成功访问的页面数：'+str(globalValues.SUCCESS_COUNT)+'。访问失败的页面数：'+str(globalValues.FAIL_COUNT)+'。正确的歌曲页面数：'+str(globalValues.SONG_PAGE_COUNT))
...
if __name__ == '__main__':
    ...
    while urlQueue:
        ...
        if (html == None):
            continue
        writeSongMsg(html)
    print(globalValues.SUCCESS_COUNT)